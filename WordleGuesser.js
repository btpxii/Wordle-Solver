...
function getExactWords() {
    ...
    let possibleWords = [];
    for (let i = 0; i < allPossibleWords.length; i++) {
        var found = [];
        // console.log("The current word being checked is " + allPossibleWords[i]);
        for (let j = 0; j < wordPos.length; j++) {
            // console.log("Comparing " + allPossibleWords[i][wordPos[j]] + " to " + given[wordPos[j]]);
            if (allPossibleWords[i][wordPos[j]] === given[wordPos[j]]) {
                // console.log("Found a match! " + allPossibleWords[i][wordPos[j]] + " is equal to " + given[wordPos[j]] + " in the word " + allPossibleWords[i]);
                found.push("t");
            }
        }
...
function getPositionWords() {
    let possibleWords = getExactWords()[0];
    let exactPos = getExactWords()[1];
    let given = [];
    for (let i = 0; i < includedInputs.length; i++) {
        given.push(includedInputs[i].value.toLowerCase());
    }
    // console.log("The given inputs are " + given);
    let wordPos = [];
    for (let i = 0; i < given.length; i++) {
    ...
    for (let i = 0; i < possibleWords.length; i++) {
        ...
        if (contains.length == wordPos.length) {
            evenMorePossibleWords.push(possibleWords[i]);
        }
    }
    return evenMorePossibleWords;
}
...
function cannotContain() {
    ...
    // console.log(notInclInput);
    for (let i = 0; i < possibleWords.length; i++) {
        let noContain = [];
        for (let j = 0; j < notInclInput.length; j++) {
            if (possibleWords[i].includes(notInclInput[j]) == false) {
                // console.log(possibleWords[i] + " doesn't contain a " + notInclInput[j]);
                noContain.push("t");
            }
        }
        if (noContain.length == notInclInput.length) {
            gottaBeOneOfThese.push(possibleWords[i]);
        }
    }
